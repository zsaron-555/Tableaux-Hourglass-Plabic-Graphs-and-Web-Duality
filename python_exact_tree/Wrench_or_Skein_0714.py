...
def _first_neighbor_slot(adj: Adjacency, node: int, neighbor: int) -> int | str:
    """Return the exact rooted half-edge slot consumed by a local rewrite.

    Endpoint names do not distinguish the two occurrences of a parallel edge.
    Saving the list index (or the ``top``/``bot`` port name) before mutation
    makes the transport replayable without reconstructing it from geometry.
    """
    neighbors = adj[node]
    if isinstance(neighbors, dict):
        for port, value in neighbors.items():
            if value == neighbor:
                return str(port)
    else:
        for index, value in enumerate(neighbors):
            if value == neighbor:
                return int(index)
    raise ValueError(f"Node {node} is not adjacent to {neighbor}; no rooted slot exists.")


def _replacement_edge_slot_record(
    adj: Adjacency,
    first_port: int,
    first_removed_endpoint: int,
    second_port: int,
    second_removed_endpoint: int,
) -> Dict[str, Any]:
    """Describe the two exact rooted slots joined by a replacement edge."""
    return {
        "edge": [int(first_port), int(second_port)],
        "endpoints": [
            {
                "vertex": int(first_port),
                "slot": _first_neighbor_slot(adj, int(first_port), int(first_removed_endpoint)),
                "replaced_neighbor": int(first_removed_endpoint),
            },
            {
                "vertex": int(second_port),
                "slot": _first_neighbor_slot(adj, int(second_port), int(second_removed_endpoint)),
                "replaced_neighbor": int(second_removed_endpoint),
            },
        ],
    }


def _assert_external_slot_transport(
    before: Adjacency,
    after: Adjacency,
    local_vertices: Iterable[int],
    relation: str,
) -> None:
    """Reject a rewrite that changes any surviving external slot structure.

    Local vertices may be deleted or change type.  Every other vertex must
    retain its valence, its tag-started list indices, and (for hourglass
    endpoints) its four-slot strand pattern.  Neighbor substitutions may only
    occupy the slots of the removed local arms.
    """
    local = {int(vertex) for vertex in local_vertices}
    before_external = set(before) - local
    after_external = set(after) - local
    if before_external != after_external:
        missing = sorted(before_external - after_external)
        added = sorted(after_external - before_external)
        raise ValueError(
            f"{relation} changed external vertices: missing={missing}, added={added}."
        )
    for node in sorted(before_external):
        old = before[node]
        new = after[node]
        if isinstance(old, dict) != isinstance(new, dict):
            raise ValueError(f"{relation} changed the external vertex type at {node}.")
        if isinstance(old, dict):
            if tuple(old.keys()) != tuple(new.keys()):
                raise ValueError(f"{relation} changed rooted port names at {node}.")
            old_pattern = tuple(getattr(old, "slot_pattern", ()))
            new_pattern = tuple(getattr(new, "slot_pattern", ()))
            if old_pattern != new_pattern:
                raise ValueError(
                    f"{relation} changed the tagged hourglass pattern at {node}: "
                    f"{old_pattern!r} -> {new_pattern!r}."
                )
        elif len(old) != len(new):
            raise ValueError(
                f"{relation} changed external valence at {node}: {len(old)} -> {len(new)}."
            )
...
def apply_hourglass_reducing_square_move(
    ...
    _assert_external_slot_transport(
        adj,
        new_adj,
        set(cycle) | set(outer),
        "four-hourglass square move",
    )
...
def detect_figure43_moves(
    ...
    """Detect executable Figure 43 rewrites in the current graph schema.

    At q=1 these are:

      * one hourglass side: 2 times the collapsed hourglass;
      * opposite hourglass sides: horizontal minus 2 times vertical.

    The adjacent-one-hourglass row has a tagged lower-valence tensor on its
    right-hand side.  It is catalogued in the relation metadata, but is not
    executable until tagged lower-valence vertices are represented explicitly.
    """
    if not node_colors:
        return []
    ...
    for quad in candidates:
        if any(v not in node_colors for v in quad):
            continue
        # Choose the local cycle from edge types and vertex colors, never from
        # screen coordinates.  The first corner is black and the cycle colors
        # alternate black-white-black-white.
        valid_orders: List[Tuple[str, Tuple[int, int, int, int]]] = []
        for candidate in itertools.permutations(quad):
            ...
            if colors != ("black", "white", "black", "white"):
                continue
            if side_types == ("hourglass", "ordinary", "ordinary", "ordinary"):
                valid_orders.append(
                    ("GPPSS_F43_single_top_hourglass", tuple(int(v) for v in candidate))
                )
            elif side_types == ("ordinary", "hourglass", "ordinary", "hourglass"):
                valid_orders.append(
                    ("GPPSS_F43_left_right_hourglasses", tuple(int(v) for v in candidate))
                )
        if not valid_orders:
            continue
        rule_name, ordered_tuple = min(valid_orders)
        ...
        if ports is None:
            continue
        if rule_name == "GPPSS_F43_single_top_hourglass":
            # The retained white/black endpoints are bottom-left/bottom-right.
            if not all(
                _colors_are_opposite(node_colors.get(port), endpoint_color)
                for port, endpoint_color in (
                    (ports["top_left"], "white"),
                    (ports["bottom_left"], "white"),
                    (ports["top_right"], "black"),
                    (ports["bottom_right"], "black"),
                )
            ):
                continue
            rhs_terms = [
                {
                    "smoothing": "collapse_to_hourglass",
...
def _figure43_collapsed_hourglass(
    white: int,
    black: int,
    white_ports: Tuple[int, int],
    black_ports: Tuple[int, int],
) -> Hourglass:
    return {
        "white": int(white),
        "black": int(black),
        "left": int(black),
        "right": int(white),
        "left_top": int(black_ports[0]),
        "left_bot": int(black_ports[1]),
        "right_top": int(white_ports[0]),
        "right_bot": int(white_ports[1]),
        "white_port0": int(white_ports[0]),
        "white_port1": int(white_ports[1]),
        "black_port0": int(black_ports[0]),
        "black_port1": int(black_ports[1]),
        "left_endpoint": "black",
        "local_case": "figure43_single_top_hourglass",
    }


def _collapse_figure43_single_top_hourglass(
    adj: Adjacency,
    remaining_hourglasses: List[Hourglass],
    match: Dict[str, Any],
) -> Tuple[Adjacency, List[Hourglass]]:
    """Apply Figure 43 row 3 directly as two times one hourglass."""
    tl, tr, br, bl = [int(v) for v in match["vertices_top_right_bottom_left"]]
    new_adj = copy.deepcopy(adj)
    ports = _figure43_external_ports(new_adj, tl, tr, br, bl)
    if ports is None:
        raise ValueError("Figure 43 single-hourglass collapse needs four external ports.")

    replace_one_neighbor(ports["top_left"], tl, bl, new_adj)
    replace_one_neighbor(ports["top_right"], tr, br, new_adj)
    for vertex in (tl, tr):
        if vertex in new_adj:
            del new_adj[vertex]

    white_ports = (int(ports["top_left"]), int(ports["bottom_left"]))
    black_ports = (int(ports["top_right"]), int(ports["bottom_right"]))
    slot_pattern = ("top", "strand:0", "strand:1", "bot")
    new_adj[bl] = HourglassPorts(
        {"top": white_ports[0], "bot": white_ports[1]},
        slot_pattern=slot_pattern,
    )
    new_adj[br] = HourglassPorts(
        {"top": black_ports[0], "bot": black_ports[1]},
        slot_pattern=slot_pattern,
    )
    removed_pair = _edge_pair(tl, tr)
    new_hgs = [
        copy.deepcopy(hg)
        for hg in remaining_hourglasses
        if hourglass_key(hg) != removed_pair
    ]
    new_hgs.append(
        _figure43_collapsed_hourglass(bl, br, white_ports, black_ports)
    )
...
def _splice_external_pair(adj: Adjacency, local_a: int, ext_a: int, local_b: int, ext_b: int) -> None:
    ...
    # Consume one concrete half-edge occurrence at each endpoint.  Replacing
    # every repeated neighbor silently collapses parallel edges and shifts the
    # rooted cyclic slots used by later sign computations.
    replace_one_neighbor(ext_a, local_a, ext_b, adj)
    replace_one_neighbor(ext_b, local_b, ext_a, adj)
...
def apply_figure43_move(
    ...
    if match.get("rule") == "GPPSS_F43_single_top_hourglass":
        if smoothing != "collapse_to_hourglass":
            raise ValueError("The single-hourglass Figure 43 row has one collapsed RHS.")
        return _collapse_figure43_single_top_hourglass(
            adj,
...
def _double_edge_hourglass_ports(
    ...
    neighbors: List[int],
    *,
    reverse_strands: bool = False,
) -> HourglassPorts:
    """Turn a tagged four-slot ordinary lens endpoint into hourglass ports.

    The two ends of an embedded bigon read its two strands in opposite local
    orientations.  ``reverse_strands`` is therefore set on the black endpoint,
    or recovered more specifically from rooted generated-edge records.
    Ordinary external ports remain in their existing tagged slots.
    """
    ...
    for neighbor in neighbors:
        if int(neighbor) == int(other):
            strand_index = 1 - strand if reverse_strands else strand
            pattern.append(f"strand:{strand_index}")
            strand += 1
        else:
...
def detect_double_edge_skein_moves(
    ...
    The stronger collapse is returned first:

      hourglass + ordinary edge = 3 * ordinary edge,
      two ordinary edges        = 2 * hourglass.

    A two-edge lens is accepted only when its repeated half-edges are adjacent
    in the tagged cyclic order at both endpoints.
    """
    matches: List[Dict[str, Any]] = []
    hourglass_pairs = _hourglass_pairs(remaining_hourglasses)
...
def apply_double_edge_skein_move(
    ...
    new_adj[black] = _double_edge_hourglass_ports(
        black,
        white,
        black_neighbors,
        reverse_strands=bool(match.get("reverse_black_strands", True)),
    )
...
def smooth_one_hourglass_embedded(
    ...
    *,
    ...
    # Use the two explicit strand slots when the full ribbon data is present.
    # ``top`` and ``bot`` are port names, not intrinsic strand identities; a
    # prior skein move may preserve the port while reversing strand:0/1.
    if (
        isinstance(adj[white], HourglassPorts)
        and isinstance(adj[black], HourglassPorts)
        and len(adj[white].slot_pattern) == 4
        and len(adj[black].slot_pattern) == 4
    ):
        white_by_strand = {
            _simple_port_strand(adj[white], port): int(adj[white][port])
            for port in ("top", "bot")
        }
        black_by_strand = {
            _simple_port_strand(adj[black], port): int(adj[black][port])
            for port in ("top", "bot")
        }
        if set(white_by_strand) != {"strand:0", "strand:1"} or set(
            black_by_strand
        ) != {"strand:0", "strand:1"}:
            raise ValueError("Wrench smoothing could not recover both rooted strand labels.")
        strand_pairs = (
            (("strand:0", "strand:0"), ("strand:1", "strand:1"))
            if smoothing == "crossing"
            else (("strand:0", "strand:1"), ("strand:1", "strand:0"))
        )
        pairings = [
            (
                white_by_strand[white_strand],
                white,
                black_by_strand[black_strand],
                black,
            )
            for white_strand, black_strand in strand_pairs
        ]
    elif smoothing == "crossing":
        # Compatibility path for small legacy fixtures that have only top/bot.
        pairings = [(w0, white, b1, black), (w1, white, b0, black)]
    else:
        pairings = [(w0, white, b0, black), (w1, white, b1, black)]

    new_adj = copy.deepcopy(adj)
    replacement_edge_slots: List[Dict[str, Any]] = []
    for first_port, first_endpoint, second_port, second_endpoint in pairings:
        # Record against the progressively mutated state.  This distinguishes
        # the first and second occurrences when both arms meet the same leaf.
        replacement_edge_slots.append(
            _replacement_edge_slot_record(
                new_adj,
                first_port,
                first_endpoint,
                second_port,
    ...
    metadata = {
        ...
        "embedding_policy": "exact_strand_and_rooted_slot_transport",
        "replacement_edges": [[int(a), int(b)] for a, _, b, _ in pairings],
        "replacement_edge_slots": replacement_edge_slots,
        ...
        for branch in beam:
            ...
            for hg in branch["remaining"]:
                for smoothing in ("crossing", "parallel"):
                    ...
                    _attach_rotation_transport(
                        move,
                        branch["adj"],
                        next_adj,
                        {int(hg["white"]), int(hg["black"])},
                        f"strategic wrench {smoothing}",
                    )
                    next_branch = {
                        "adj": next_adj,
...
def tagged_rotation_state_sha256(adj: Adjacency) -> str:
    """Stable digest of every tag-started half-edge slot in one web state."""
    payload = json.dumps(tagged_rotation_tuple(adj), separators=(",", ":"))
    return hashlib.sha256(payload.encode("utf-8")).hexdigest()


def rotation_transport_certificate(
    before: Adjacency,
    after: Adjacency,
    local_vertices: Iterable[int],
    relation: str,
) -> Dict[str, Any]:
    """Create an auditable before/after certificate for a local relation."""
    local = {int(vertex) for vertex in local_vertices}
    _assert_external_slot_transport(before, after, local, relation)
    changed: List[Dict[str, Any]] = []
    before_records = {int(record[1]): record for record in tagged_rotation_tuple(before)}
    after_records = {int(record[1]): record for record in tagged_rotation_tuple(after)}
    for node in sorted(set(before_records) | set(after_records)):
        old = before_records.get(node)
        new = after_records.get(node)
        if old != new:
            changed.append(
                {
                    "vertex": int(node),
                    "local": node in local,
                    "before": None if old is None else list(old),
                    "after": None if new is None else list(new),
                }
            )
    return {
        "schema": "exact_tagged_rotation_transport_v1",
        "relation": str(relation),
        "local_vertices": sorted(local),
        "before_sha256": tagged_rotation_state_sha256(before),
        "after_sha256": tagged_rotation_state_sha256(after),
        "changed_vertices": changed,
        "external_slot_structure_verified": True,
    }


def _attach_rotation_transport(
    move: Dict[str, Any],
    before: Adjacency,
    after: Adjacency,
    local_vertices: Iterable[int],
    ...
    move["rotation_transport"] = rotation_transport_certificate(
        before,
        after,
        local_vertices,
        relation,
...
def _verify_rotation_checkpoint(
    ...
    certificate = move.get("rotation_transport")
    if not isinstance(certificate, dict):
        return
    expected = certificate.get(f"{stage}_sha256")
    if not expected:
        raise ValueError(f"Rotation certificate has no {stage} checkpoint.")
    actual = tagged_rotation_state_sha256(adj)
    if actual != expected:
        raise ValueError(
            f"Ribbon replay mismatch {stage} {certificate.get('relation', 'relation')}: "
...
def graph_json_from_term_side(
    ...
    nodes = []
    for node in sorted(adj):
        color = (node_colors or {}).get(int(node))
        if not color:
            color = "black" if int(node) in boundary_labels else ""
        nodes.append({"id": int(node), "color": str(color)})

    edges: Set[Tuple[int, int]] = set()
    for u, neighbors in adj.items():
        if isinstance(neighbors, MutableMapping):
            iterable = [v for v in neighbors.values() if v is not None]
        else:
            iterable = neighbors
        for v in iterable:
            if v is None:
    ...
    return {
        "nodes": nodes,
        "boundary": [
            {"node": int(node), "label": int(label)}
            for node, label in sorted(boundary_labels.items(), key=lambda item: item[1])
        ],
        "edges": [
            {"src": int(u), "dst": int(v)}
        ...
        "hourglasses": [
            {"white": int(hg["white"]), "black": int(hg["black"])}
            for hg in hourglasses
            if int(hg["white"]) in adj and int(hg["black"]) in adj
        ],
    }
...
def lemma48_pair_match_for_term(
    term: Dict[str, Any],
    x_boundary_labels: BoundaryLabels,
    w_boundary_labels: BoundaryLabels,
    x_node_colors: Optional[NodeColors],
    ...
    matches = relation_rules.detect_sl4_lemma48_zero_pair(w_graph, x_graph, max_matches=1)
    return matches[0] if matches else None
...
def pair_state_key(state: Dict[str, Any]) -> Tuple[Any, ...]:
    """Structural key for one algebraic active state.

    Discharged terms already contribute zero, and relation histories are proof
    witnesses rather than algebraic data.  Excluding both prevents a forward
    reduction from being revisited merely under a different history.
    """
    return (
        "pair-state-v1",
        tuple(
    ...
    *,
    ...
    for term in terms:
        ...
        if common:
            ...
            lemma49_match = lemma49_pair_match_for_term(
                term,
                x_boundary_labels,
                w_boundary_labels,
                x_node_colors,
                w_node_colors,
            ) if use_lemma49 else None
            if lemma49_match:
                source = lemma49_match.get("source", {})
                discharged.append(
                    {
                        "coeff": term["coeff"],
                        "common_forks": [],
                        "history": term.get("history", []),
                        "reason": "lemma49_zero",
                        "lemma49_match": lemma49_match,
                        "lemma49_rule_id": lemma49_match.get("rule_id"),
                        "lemma49_case": source.get("case") or lemma49_match.get("reason"),
                        "lemma49_boundary_labels": lemma49_match.get("boundary_labels", []),
                    }
                )
            else:
                lemma48_match = lemma48_pair_match_for_term(
                    term,
                    x_boundary_labels,
                    w_boundary_labels,
                    x_node_colors,
                    w_node_colors,
                ) if use_lemma48 else None
                if lemma48_match:
                    source = lemma48_match.get("source", {})
                    discharged.append(
                        {
                            "coeff": term["coeff"],
                            "common_forks": [],
                            "history": term.get("history", []),
                            "reason": "lemma48_zero",
                            "lemma48_match": lemma48_match,
                            "lemma48_rule_id": lemma48_match.get("rule_id"),
                            "lemma48_case": source.get("case") or lemma48_match.get("reason"),
                            "lemma48_boundary_labels": lemma48_match.get("boundary_labels", []),
                        }
                    )
                else:
                    active.append(term)
...
def expand_pair_term(
    ...
    *,
    ...
    for smoothing in ("crossing", "parallel"):
        ...
        before_adj = term["x_adj"] if side == "X" else term["w_adj"]
        after_adj = child_x_adj if side == "X" else child_w_adj
        _attach_rotation_transport(
            move,
            before_adj,
            after_adj,
            {int(hg["white"]), int(hg["black"])},
            f"{side} wrench {smoothing}",
        )
        children.append(
            {
                "x_adj": child_x_adj,
...
def expand_pair_term_by_figure43(
    ...
    for rhs in match.get("rhs_terms", []):
        ...
        _attach_rotation_transport(
            ...
            before_adj,
            after_adj,
            {int(v) for v in match["vertices_top_right_bottom_left"]},
            f"{side} Figure 43 {smoothing}",
        )
        children.append(
            {
                "x_adj": child_x_adj,
...
def expand_pair_term_by_square_move(
    ...
    _attach_rotation_transport(
        ...
        after_adj,
        {
            ...
            *(int(vertex) for vertex in match["outer_vertices"]),
        },
        f"{side} four-hourglass square move",
    )
    return [
        {
            "x_adj": child_x_adj,
...
def _history_live_replacement_lens_slots(
    history: Iterable[Dict[str, Any]],
    adj: Adjacency,
    white: int,
    black: int,
    ...
    """Recover live generated lens occurrences by their rooted endpoint slots."""
    target = _edge_pair(white, black)
    records: List[Dict[str, int]] = []
    for move_index, move in enumerate(history):
        if side is not None and move.get("side") not in {None, side}:
            continue
        for record in move.get("replacement_edge_slots", []):
            edge = record.get("edge", [])
            if len(edge) != 2 or _edge_pair(int(edge[0]), int(edge[1])) != target:
                continue
            endpoints = {
                int(item["vertex"]): item
                for item in record.get("endpoints", [])
                if "vertex" in item
            }
            if set(endpoints) != {int(white), int(black)}:
                continue
            normalized: Dict[int, int] = {}
            for vertex, other in ((int(white), int(black)), (int(black), int(white))):
                slot = endpoints[vertex].get("slot")
                neighbors = adj.get(vertex)
                if (
                    not isinstance(slot, int)
                    or not isinstance(neighbors, list)
                    or not 0 <= slot < len(neighbors)
                    or int(neighbors[slot]) != other
                ):
                    break
                normalized[vertex] = slot
            else:
                records.append(
                    {
                        "move_index": int(move_index),
                        "white_slot": normalized[int(white)],
                        "black_slot": normalized[int(black)],
                    }
                )
    return records


def _exact_black_lens_strand_reversal(
    adj: Adjacency,
    white: int,
    black: int,
    generated_slots: Iterable[Dict[str, int]],
) -> Optional[bool]:
    """Determine black strand numbering from a concrete paired occurrence."""
    white_neighbors = adj.get(int(white))
    black_neighbors = adj.get(int(black))
    if not isinstance(white_neighbors, list) or not isinstance(black_neighbors, list):
        return None
    white_slots = [
        index for index, neighbor in enumerate(white_neighbors) if int(neighbor) == int(black)
    ]
    black_slots = [
        index for index, neighbor in enumerate(black_neighbors) if int(neighbor) == int(white)
    ]
    if len(white_slots) != 2 or len(black_slots) != 2:
        return None
    candidates: Set[bool] = set()
    for record in generated_slots:
        try:
            white_index = white_slots.index(int(record["white_slot"]))
            black_index = black_slots.index(int(record["black_slot"]))
        except (KeyError, ValueError):
            continue
        candidates.add(black_index != white_index)
    if len(candidates) == 1:
...
def expand_pair_term_by_double_edge_skein(
    ...
    if str(match["kind"]) == "double_edge":
        active_adj = term["x_adj"] if side == "X" else term["w_adj"]
        rooted_lens_records = _history_live_replacement_lens_slots(
            term.get("history", []),
            active_adj,
            int(match["white"]),
            int(match["black"]),
        ...
        exact_reversal = _exact_black_lens_strand_reversal(
            active_adj,
            int(match["white"]),
            int(match["black"]),
            rooted_lens_records,
        )
        effective_match["reverse_black_strands"] = (
            True if exact_reversal is None else exact_reversal
        )
        effective_match["exact_lens_slot_record_count"] = len(rooted_lens_records)
        effective_match["exact_lens_slot_transport_resolved"] = exact_reversal is not None
        effective_match["lens_rotation_source"] = (
            "rooted_replacement_slots"
            if exact_reversal is not None
            else "canonical_embedded_bigon"
        )
    multiplier = int(effective_match["coefficient_multiplier"])
    if side == "X":
    ...
    move = {
        ...
        "side": side,
        "rule": str(effective_match["rule"]),
        "kind": str(effective_match["kind"]),
        "vertices": [int(effective_match["white"]), int(effective_match["black"])],
        "white": int(effective_match["white"]),
        "black": int(effective_match["black"]),
        "smoothing": str(effective_match["kind"]),
        "coefficient_multiplier": multiplier,
    }
    if str(effective_match["kind"]) == "hourglass_plus_edge":
        move["hourglass"] = [int(effective_match["white"]), int(effective_match["black"])]
    for field in (
        "external_white",
        "external_black",
        "exact_lens_slot_record_count",
    ):
        if field in effective_match:
            move[field] = int(effective_match[field])
    for field in ("reverse_black_strands", "exact_lens_slot_transport_resolved"):
        if field in effective_match:
            move[field] = bool(effective_match[field])
    if "lens_rotation_source" in effective_match:
        move["lens_rotation_source"] = str(effective_match["lens_rotation_source"])
    before_adj = term["x_adj"] if side == "X" else term["w_adj"]
    after_adj = child_x_adj if side == "X" else child_w_adj
    _attach_rotation_transport(
        move,
        before_adj,
        after_adj,
        {int(effective_match["white"]), int(effective_match["black"])},
        f"{side} {effective_match['kind']}",
    )
    return [
        {
            "x_adj": child_x_adj,
    ...
    *,
    ...
    for rhs in match.get("rhs_terms", []):
        ...
        _attach_rotation_transport(
            ...
            term["x_adj"],
            child_x_adj,
            {int(match["white"]), int(match["black"])},
            f"X antisymmetrizer {move['smoothing']}",
        )
        children.append(
            {
                "x_adj": child_x_adj,
...
def choose_pair_successors(
    ...
    *,
    ...
    for term_idx, term in enumerate(active):
        ...
        relation_successor_count = len(successors)
        ...
        if len(successors) > relation_successor_count:
            continue
...
def replay_pair_history(
    ...
    for move in history:
        ...
        _verify_rotation_checkpoint(
            move,
            current_x if side == "X" else current_w,
            "before",
        )
        ...
        if move.get("phase") == "double_edge_skein":
            ...
            if "reverse_black_strands" in move:
                match["reverse_black_strands"] = bool(move["reverse_black_strands"])
            if side == "X":
    ...
    *,
    ...
    for step in range(x_resolution_steps + 1):
        ...
        candidates = [
            ...
            if pair_state_key(state) not in seen_x_resolution_states
        ]
        if not candidates:
            break
        ...
        seen_x_resolution_states.update(pair_state_key(state) for state in beam)
