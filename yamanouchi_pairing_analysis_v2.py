...
Reports 1, 2, 3, and 5 exclude any w_word that has at least one pairing
whose final_pairing_value is not an integer (e.g. "timeout" or a non-whole
fraction). Report 4 is unchanged.
...
def pairing_is_integer(value):
    """Return True when value is an exact integer (including 0)."""
    ...
    with open(path, newline='', encoding='utf-8-sig') as f:
        ...
        for line_num, row in enumerate(reader, start=2):
            ...
            try:
                ...
                is_integer = pairing_is_integer(value)
            except Exception:
                print(
                    "Warning: could not parse final_pairing_value {!r} on "
                    "line {}; treating as non-integer pairing.".format(
                        h_raw, line_num
                    ),
                    file=sys.stderr,
                )
                value = h_raw
                is_integer = False
            rows.append((w_raw, x_raw, value, is_integer))
    return rows


def w_words_with_noninteger_pairing(rows):
    """Return the set of w_words that have at least one non-integer pairing."""
    bad = set()
    for w_raw, _x_raw, _value, is_integer in rows:
        if not is_integer:
            bad.add(w_raw)
    return bad
...
def group_by_w(rows):
    ...
    Only integer pairings are included; non-integer rows still cause the
    w_word to be flagged via w_words_with_noninteger_pairing().
    """
    groups = {}
    for w_raw, x_raw, value, is_integer in rows:
        if not is_integer:
            continue
        if value == 0:
            continue
...
def main():
    ...
    noninteger_w_words = w_words_with_noninteger_pairing(rows)
    groups = group_by_w(rows)

    def exclude_noninteger_w_words(triples):
        return [(w, x, v) for (w, x, v) in triples if w not in noninteger_w_words]
    ...
    # Categories 1, 2, 3, and 5 omit w_words with any non-integer pairing.
    A = exclude_noninteger_w_words(A)
    B = exclude_noninteger_w_words(B)
    single_not_pm1 = exclude_noninteger_w_words(single_not_pm1)
