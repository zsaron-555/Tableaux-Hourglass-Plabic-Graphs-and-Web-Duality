...
_PAIR_PROGRESS_CHECKPOINT: Optional[Dict[str, Any]] = None


def _set_pair_progress_checkpoint(
    phase: str,
    step: int,
    state: Dict[str, Any],
    ...
    x_node_xy: Optional[NodeXY],
    w_node_xy: Optional[NodeXY],
) -> None:
    """Retain the latest completed search state for timeout diagnostics.

    The state is intentionally kept by reference and serialized only if the
    runner asks for it after a timeout.  This avoids deep-copying a potentially
    large beam state after every completed search step.
    """
    global _PAIR_PROGRESS_CHECKPOINT
    _PAIR_PROGRESS_CHECKPOINT = {
        "phase": str(phase),
        "step": int(step),
        "state": state,
        "x_boundary_labels": x_boundary_labels,
        "w_boundary_labels": w_boundary_labels,
        "x_node_colors": x_node_colors or {},
        "w_node_colors": w_node_colors or {},
        "x_node_xy": x_node_xy or {},
        "w_node_xy": w_node_xy or {},
        "captured_at_unix": time.time(),
    }


def clear_pair_progress_checkpoint() -> None:
    global _PAIR_PROGRESS_CHECKPOINT
    _PAIR_PROGRESS_CHECKPOINT = None


def _json_safe_pair_value(value: Any) -> Any:
    if value is None or isinstance(value, (str, int, float, bool)):
        return value
    if isinstance(value, dict):
        return {
            str(key): _json_safe_pair_value(item)
            for key, item in value.items()
        }
    if isinstance(value, (list, tuple)):
        return [_json_safe_pair_value(item) for item in value]
    if isinstance(value, (set, frozenset)):
        return sorted(
            (_json_safe_pair_value(item) for item in value),
            key=lambda item: repr(item),
        )
    return repr(value)


def get_pair_progress_checkpoint() -> Optional[Dict[str, Any]]:
    """Serialize the latest completed best state after an interrupted search."""
    checkpoint = _PAIR_PROGRESS_CHECKPOINT
    if checkpoint is None:
        return None

    state = checkpoint["state"]
    x_boundary_labels = checkpoint["x_boundary_labels"]
    w_boundary_labels = checkpoint["w_boundary_labels"]
    x_node_colors = checkpoint["x_node_colors"]
    w_node_colors = checkpoint["w_node_colors"]
    x_node_xy = checkpoint["x_node_xy"]
    w_node_xy = checkpoint["w_node_xy"]
    active_terms: List[Dict[str, Any]] = []
    for term_index, raw_term in enumerate(state.get("active", []), start=1):
        term = normalize_pair_term(raw_term)
        x_remaining = term["x_remaining"]
        w_remaining = term["w_remaining"]
        x_internal_black = sorted(
            int(node)
            for node in term["x_adj"]
            if int(node) not in x_boundary_labels
            and x_node_colors.get(int(node)) == "black"
        )
        w_internal_black = sorted(
            int(node)
            for node in term["w_adj"]
            if int(node) not in w_boundary_labels
            and w_node_colors.get(int(node)) == "black"
        )
        active_terms.append(
            {
                "term_index": term_index,
                "coefficient": int(term.get("coeff", 0)),
                "history": _json_safe_pair_value(term.get("history", [])),
                "x_adjacency": _json_safe_pair_value(term["x_adj"]),
                "w_adjacency": _json_safe_pair_value(term["w_adj"]),
                "x_edges": [list(edge) for edge in get_edge_tuple(term["x_adj"])],
                "w_edges": [list(edge) for edge in get_edge_tuple(term["w_adj"])],
                "x_remaining_hourglasses": _json_safe_pair_value(x_remaining),
                "w_remaining_hourglasses": _json_safe_pair_value(w_remaining),
                "x_remaining_hourglass_count": len(x_remaining),
                "w_remaining_hourglass_count": len(w_remaining),
                "x_internal_black_vertices": x_internal_black,
                "w_internal_black_vertices": w_internal_black,
                "x_internal_black_vertex_count": len(x_internal_black),
                "w_internal_black_vertex_count": len(w_internal_black),
            }
        )

    return {
        "schema": "sl4_pairing_timeout_state_v1",
        "phase": checkpoint["phase"],
        "last_completed_step": checkpoint["step"],
        "captured_at_unix": checkpoint["captured_at_unix"],
        "active_term_count": len(active_terms),
        "discharged_term_count": len(state.get("discharged", [])),
        "score": _json_safe_pair_value(state.get("score")),
        "expanded_side": state.get("expanded_side"),
        "expanded_hourglass": _json_safe_pair_value(state.get("expanded_hourglass")),
        "expanded_relation": state.get("expanded_relation"),
        "expanded_rule": state.get("expanded_rule"),
        "expanded_vertices": _json_safe_pair_value(state.get("expanded_vertices")),
        "x_boundary_labels": _json_safe_pair_value(x_boundary_labels),
        "w_boundary_labels": _json_safe_pair_value(w_boundary_labels),
        "x_node_colors": _json_safe_pair_value(x_node_colors),
        "w_node_colors": _json_safe_pair_value(w_node_colors),
        "x_node_xy": _json_safe_pair_value(x_node_xy),
        "w_node_xy": _json_safe_pair_value(w_node_xy),
...
def prove_pair_value_by_x_component_coloring(
    ...
    *,
    ...
    """
    clear_pair_progress_checkpoint()
    initial_remaining = len(x_hourglasses) + (len(w_hourglasses) if allow_w_wrench else 0)
...
def pair_state_reduction_key(state: Dict[str, Any]) -> Tuple[Any, ...]:
    """Projective canonical key used only to prevent reduction cycles."""
    entries = [
...
def pair_state_reduction_fingerprint(state: Dict[str, Any]) -> Tuple[Any, ...]:
    """Cheap projective fingerprint used before exact cycle comparison."""
    entries = [
        (pair_term_fingerprint(term), int(term["coeff"]))
        for term in state.get("active", [])
        if int(term["coeff"]) != 0
    ]
    entries.sort(key=lambda item: repr(item[0]))
    if not entries:
        return ("pair-reduction-fingerprint-v1", ())
    divisor = 0
    for _, coeff in entries:
        divisor = math.gcd(divisor, abs(coeff))
    divisor = max(1, divisor)
    ...
    if normalized[0][1] < 0:
        normalized = [(key, -coeff) for key, coeff in normalized]
    return ("pair-reduction-fingerprint-v1", tuple(normalized))
...
def x_resolution_complexity(
    ...
    """Complexity used to put monotone X reductions first."""
...
def cycle_guard_successors(
    ...
    """
    ancestor_index = {
        fingerprint: set(exact_keys)
        for fingerprint, exact_keys in parent.get(
            "_x_reduction_ancestor_index", {}
        ).items()
    }
    parent_fingerprint = pair_state_reduction_fingerprint(parent)
    parent_keys = ancestor_index.setdefault(parent_fingerprint, set())
    parent_keys.add(pair_state_reduction_key(parent))
    ...
    for child in successors:
        child_fingerprint = pair_state_reduction_fingerprint(child)
        possible_ancestors = ancestor_index.get(child_fingerprint)
        if possible_ancestors is not None:
            child_key = pair_state_reduction_key(child)
            if child_key in possible_ancestors:
                rejected += 1
                continue
        ...
        if child_complexity < parent_complexity:
            ...
            progress_class = 0
            worsening += 1
        child["_x_reduction_ancestor_index"] = {
            fingerprint: frozenset(exact_keys)
            for fingerprint, exact_keys in ancestor_index.items()
        }
