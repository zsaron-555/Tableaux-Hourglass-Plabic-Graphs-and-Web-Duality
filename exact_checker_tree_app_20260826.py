...
<title>Wrench Pairing Explorer — Exact Tagged Tree</title>
<style>
...
<header><h1>Wrench Pairing Explorer</h1><p class="muted">Authoritative tagged pairing value, branch ledger, and exact reduction tree from one checker run.</p>
<p class="muted">Graph data: <span class="mono">{html.escape(str(catalogue_graph_dir()))}</span> · <a href="{GRAPH_DATA_DOWNLOAD_URL}">Download 4x4 graph data</a>. Extract the ZIP anywhere, then start this app with <span class="mono">--project-root "/path/to/folder-containing-4x4_All_graph_data"</span>.</p>
...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if parsed.path == "/health":
            self.send_html('{"status":"ready","app":"combined_exact_pairing_explorer"}')
            return
...
def main(argv: Iterable[str] | None = None) -> None:
    ...
    print(f"Combined Wrench Pairing Explorer running at http://{args.host}:{args.port}/")
    print("Local tagging convention: GPPSS. Terminal conversion only: FLL.")
    print(f"Using graph data from {catalogue_graph_dir()}")
